...
ESX.RegisterServerCallback('esx_vigneronjob:getChestWeapons', function(source, cb)
	TriggerEvent('esx_datastore:getSharedDataStore', 'society_vigne', function(store)
...
ESX.RegisterServerCallback('esx_vigneronjob:addChestWeapon', function(source, cb, weaponName, removeWeapon)
	local xPlayer = ESX.GetPlayerFromId(source)
...
ESX.RegisterServerCallback('esx_vigneronjob:removeChestWeapon', function(source, cb, weaponName)
	local xPlayer = ESX.GetPlayerFromId(source)
...
ESX.RegisterServerCallback('esx_vigneronjob:buyJobVehicle', function(source, cb, vehicleProps, type)
	...
	if price == 0 then
		print(('esx_vigneronjob: %s attempted to exploit the shop! (invalid vehicle model)'):format(xPlayer.identifier))
		cb(false)
	else
...
function getPriceFromHash(vehicleHash, jobGrade, type)
	local vehicles = Config.VehiclesList
...
RegisterNetEvent('esx_vigneronjob:forceBlip')
AddEventHandler('esx_vigneronjob:forceBlip', function()
	TriggerClientEvent('esx_vigneronjob:updateBlip', -1)
end)
...
ESX.RegisterServerCallback("esx_vigneronjob:pedExists", function(source, cb)
...
RegisterServerEvent("esx_vigneronjob:giveWeapon")
AddEventHandler("esx_vigneronjob:giveWeapon", function(weapon)
...
RegisterServerEvent('checkvigne')
AddEventHandler('checkvigne', function()
	local xPlayers = ESX.GetPlayers()
	for i=1, #xPlayers, 1 do
		local xPlayer = ESX.GetPlayerFromId(xPlayers[i])
		if xPlayer.job.name == 'vigne' then
			vigneConnected = "✔️"
			TriggerClientEvent("vigneOnline", vigneConnected)
		end	
	end
end)

RegisterServerEvent('esx_vigneronjob:buyItem')
AddEventHandler('esx_vigneronjob:buyItem', function(itemName, price, itemLabel)
