...
local HasAlreadyEnteredMarker, isDead, isHandcuffed, hasAlreadyJoined, playerInService, OnlinePolice = false, false, false, false, false, false
...
function OpenCloakroomMenu()
	...
	local elements = {
		...
		{label = 'Tenue Cadet', uniform = 'cadet' },
		{label = 'Tenue Officier I', uniform = 'officier_1' },
		{label = 'Tenue Officier II', uniform = 'officier_2' },
		{label = 'Tenue Officier III', uniform = 'officier_3' },
		{label = 'Tenue Officier III+1', uniform = 'officier_3_1' },
		{label = 'Tenue Sergent I', uniform = 'sergent_1' },
		{label = 'Tenue Sergent II', uniform = 'sergent_2' },
		{label = 'Tenue Lieutenant I', uniform = 'lieutenant_1' },
		{label = 'Tenue Lieutenant II', uniform = 'lieutenant_2' },
		{label = 'Tenue Capitaine', uniform = 'capitaine' },
		{label = 'Tenue Commandant', uniform = 'commandant' },
		{label = 'Tenue SWAT', uniform = 'tenu_swatgr'},
		{label = 'Berret grader', uniform = 'berret_grader'},
		{label = 'Casquette', uniform = 'casquette_grader'},
...
function OpenPoliceActionsMenu()
	...
	ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'police_actions', {
		...
		align    = 'top-left',
		elements = {
			{label = ('Demande de renforts<span>'..Emoticon.Droite..'</span>'), value = 'renfort'},
			{label = (_U('citizen_interaction')..'<span>'..Emoticon.Droite..'</span>'), value = 'citizen_interaction'},
			{label = (_U('vehicle_interaction')..'<span>'..Emoticon.Droite..'</span>'), value = 'vehicle_interaction'},
			{label = (_U('object_spawner')..'<span>'..Emoticon.Droite..'</span>'), value = 'object_spawner'}
	}}, function(data, menu)
		if data.current.value == 'citizen_interaction' then
			local elements = {
				{label = ('Carte d\'identité<span>'..Emoticon.Loupe..'</span>'), value = 'identity_card'},
				{label = ('Fouiller<span>'..Emoticon.Loupe..'</span>'), value = 'search'},
				{label = (_U('handcuff')..'<span>'..Emoticon.Menotte..'</span>'), value = 'handcuff'},
				{label = (_U('drag')..'<span>'..Emoticon.Pied..'</span>'), value = 'drag'},
				{label = (_U('put_in_vehicle')..'<span>'..Emoticon.Donner..'</span>'), value = 'put_in_vehicle'},
				{label = (_U('out_the_vehicle')..'<span>'..Emoticon.Recevoir..'</span>'), value = 'out_the_vehicle'},
				{label = (_U('fine_personalised')..'<span>'..Emoticon.Dollard..'</span>'), value = 'fine_personalised'},
				{label = (_U('fine')..'<span>'..Emoticon.Dollard..'</span>'), value = 'fine'},
				{label = (_U('unpaid_bills')..'<span>'..Emoticon.Feuille..'</span>'), value = 'unpaid_bills'}
			}

			if Config.EnableLicenses and ESX.PlayerData.job.grade_name == 'boss' or ESX.PlayerData.job.grade_name == 'lieutenant' then
				table.insert(elements, {label = ('Donner le PPA<span>'..Emoticon.Permis..'</span>'), value = 'ppa'})
			end

			if Config.EnableLicenses then
				table.insert(elements, {label = (_U('license_check')..'<span>'..Emoticon.Dossier..'</span>'), value = 'license'})
			end
			...
			ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'citizen_interaction', {
				...
				if closestPlayer ~= -1 and closestDistance <= 3.0 then
					...
					elseif action == 'fine_personalised' then
		...
		elseif data.current.value == 'vehicle_interaction' then
			...
			if DoesEntityExist(vehicle) then
				table.insert(elements, {label = (_U('vehicle_info')..'<span>'..Emoticon.Loupe..'</span>'), value = 'vehicle_infos'})
				table.insert(elements, {label = _U('pick_lock'), value = 'hijack_vehicle'})
				table.insert(elements, {label = _U('impound'), value = 'impound'})
			end

			table.insert(elements, {label = (_U('search_database')..'<span>'..Emoticon.Loupe..'</span>'), value = 'search_database'})
		...
		elseif data.current.value == 'renfort' then
			local elements  = {}
			local playerPed = PlayerPedId()
			table.insert(elements, {label = ('Petite demande<span>'..Emoticon.Couleur.Vert..'</span>'),value = 'petite_demande'})
			table.insert(elements, {label = ('Demande importante<span>'..Emoticon.Couleur.Orange..'</span>'), value = 'demande_importante'})
			table.insert(elements, {label = ('Toutes les unités demandées !<span>'..Emoticon.Couleur.Rouge..'</span>'), value = 'omgad'})
		...
		elseif data.current.value == 'object_spawner' then
			ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'citizen_interaction', {
				...
				align    = 'top-left',
				elements = {
					{label = (_U('cone')..'<span>'..Emoticon.Objet..'</span>'), model = 'prop_roadcone02a'},
					{label = (_U('barrier')..'<span>'..Emoticon.Objet..'</span>'), model = 'prop_barrier_work05'},
					{label = (_U('spikestrips')..'<span>'..Emoticon.Objet..'</span>'), model = 'p_ld_stinger_s'}
			}}, function(data2, menu2)
				local playerPed = PlayerPedId()
...
function OpenFineMenu(player)
	ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'fine', {
		...
		align    = 'top-left',
		elements = {
			{label = "Contravention", value = 0},
			{label = "Infractions / Délits",   value = 1},
			{label = "Crimes", value = 2},
	}}, function(data, menu)
...
Citizen.CreateThread(function()
	while true do
		local PlayerData = ESX.GetPlayerData()
		if PlayerData.job.name == ('police' or 'ambulance') then
...
AddEventHandler('renfort:setBlip', function(coords, raison)
	if raison == 'petit' then
		...
		ESX.ShowAdvancedNotification('LSPD INFORMATIONS', '~b~Demande de renforts', 'Demande de renforts demandé.\nRéponse: ~g~CODE-2\n~w~Importance: ~g~Légère.', 'CHAR_CALL911', 8)
		Wait(1000)
...
AddEventHandler('esx_policejob:updateBlip', function()
	...
	--	print("A le job police")
	--	if GetInventoryItem(Config.ItemForJobBlip).count >= 1 then
	--		print("A l'item phone")
	--		ESX.TriggerServerCallback('esx_society:getOnlinePlayers', function(players)
	--			for i=1, #players, 1 do
	--				if players[i].job.name == 'police' then
	--					local id = GetPlayerFromServerId(players[i].source)
	--					print("il et policier")
	--					if NetworkIsPlayerActive(id) and GetPlayerPed(id) ~= PlayerPedId() then
	--						createBlip(id)
	--						print("blips créer sur le joueur")
	--					end
	--				end
	--			end
	--		end)
...
Citizen.CreateThread(function()
	RequestStreamedTextureDict("blips")
    while not HasStreamedTextureDictLoaded("blips") do
        Wait(0)
	end
	for k,v in pairs(Config.PoliceStations) do
		...
		exports['blip_info']:SetBlipInfoTitle(blip, "Mission row police departement", false)
		exports['blip_info']:SetBlipInfoImage(blip, "blips", "lspd")
		exports['blip_info']:AddBlipInfoText(blip, "Type:", "~b~Service public")
		exports['blip_info']:AddBlipInfoText(blip, "Safe zone:", "~g~Oui")
		exports['blip_info']:AddBlipInfoHeader(blip, "")
		exports['blip_info']:AddBlipInfoText(blip, "Poste central des force de l'ordre de la ville de Los Santos")
	end
end)
