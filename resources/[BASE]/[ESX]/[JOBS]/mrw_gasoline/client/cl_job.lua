...
local positionJob =  {
	RaffineurAction = {2744.24, 1487.32, 29.79},  
	Garage = {2748.25, 1453.77, 23.5},  
	SpawnPoint = {2772.01, 1443.72, 23.52},  
	DeletePoint = {2772.01, 1443.72, 23.52}, 
	Vehicle = 'phantom',
	JobClothe = {
		male = {
                ...
                if ESX.PlayerData.job ~= nil and ESX.PlayerData.job.grade_name == 'boss' then 
	                RageUI.Button("Action Patron", nil, {}, true, function(Hovered, Active, Selected)
	                	if (Selected) then 
		                	...
		                	Citizen.Wait(25)    	
		    	            TriggerEvent('esx_society:openBossMenu', 'fueler', function(data, menu) end)
		    	        end
	                end)
	            end
	        end, function()
            ...
            RageUI.DrawContent({ header = true, glare = true, instructionalButton = true }, function()
                RageUI.Button("Sortir un véhicule du garage", nil, {}, true, function(Hovered, Active, Selected)
               	    if (Selected) then 
						...
						ESX.Game.SpawnVehicle(model, coord, 343.41, function(vehicle)
							ESX.Game.SpawnVehicle("tanker", coord, 343.41, function(trailer)
