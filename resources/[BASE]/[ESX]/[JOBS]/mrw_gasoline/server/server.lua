			...
			if GasolineReserve >= amountLiter then 
				if amountLiter + currentFuel <= 100 then
					if xPlayer.getMoney() >= priceLiter then 
					   ...
					   TriggerClientEvent('esx:showNotification', source, "Vous ne disposez pas d'assez d'argent !")
					end   
				else
					--xPlayer.showNotification("Votre véhicule ne peut pas recevoir autant de litre !")
					TriggerClientEvent('esx:showNotification', source, "Votre véhicule ne peut pas recevoir autant de litre !")

				end
			else
				--xPlayer.showNotification("La station ne dispose pas d'une reserve suffisante !")
				TriggerClientEvent('esx:showNotification', source, "La station ne dispose pas d'une reserve suffisante !")
			end
		end
	else 
...
AddEventHandler('mrw_gasoline:payGasole', function(id, amountLiter, priceLiter)
    ...
    if priceLiter ~= 0 or priceLiter ~= -1 then 
		if gasolineFile[id].id == id then 
			...
			TriggerClientEvent('esx:showNotification', source, "Vous venez de mettre de l'essence ~r~-"..priceLiter.."$")
		end
	else
		print("Le joueurs : "..xPlayer.identifier.." triche")
	end	
    ...
    if xPlayer.job.name == 'fueler' then 
		if gasolineFile[id].id == id then 
        	TriggerEvent('esx_addonaccount:getSharedAccount', 'society_fueler', function(account)
				...
				TriggerClientEvent('esx:showNotification', source, "Vous avez gagné : "..salary.." ~g~$~w~")
				TriggerClientEvent('esx:showNotification', source, "Vous entreprise a gagné : "..salaryentreprise.." ~g~$~w~")
