...
AddEventHandler('GangsBuilderJob:handcuff', function(target)
	TriggerClientEvent('GangsBuilder:handcuff', target)
end)

RegisterNetEvent('GangsBuilderJob:drag')
AddEventHandler('GangsBuilderJob:drag', function(target)
	TriggerClientEvent('GangsBuilder:drag', target, source)
end)
	...
	TriggerEvent('esx_addoninventory:getSharedInventory', 'society_' .. xPlayer.job2.name, function(inventory)
		...
		if count > 0 and inventoryItem.count >= count then
			...
			if sourceItem.limit ~= -1 and (sourceItem.count + count) > sourceItem.limit then
				...
				TriggerClientEvent('esx:showNotification', source, _U('have_withdrawn', count, inventoryItem.label))
			end
		else
			TriggerClientEvent('esx:showNotification', source, _U('quantity_invalid'))
		end
