...
AddEventHandler('esx_truck_inventory:getInventoryLoaded', function(inventory,weight)
  ...
  TriggerServerEvent("esx_truck_inventory:getOwnedVehicule")
	...
	table.insert(elements, {
      label     = 'Déposer <span>'..Emoticon.Loupe..'</span>',
      count     = 0,
      value     = 'deposit',
    })
	...
	if inventory ~= nil and #inventory > 0 then
		...
		if inventory[i].type == 'item_standard' then
		      table.insert(elements, {
		        label     = (inventory[i].label..'<span>'..math.floor(inventory[i].count)..'</span>'),
	...
	ESX.UI.Menu.Open(
	  ...
	  {
	    title    = 'Contenu du coffre (max '..MaxVh..'Kg)',
	    align    = 'top-left',
	    elements = elements,
	  },
	  function(data, menu)
	  	if data.current.value == 'deposit' then
				...
				if PlayerData.accounts[i].name == 'black_money' then
				  -- if PlayerData.accounts[i].money > 0 then
				    table.insert(elem, {
				      ...
				      label     = (PlayerData.accounts[i].label .. '<span>'.. math.floor(PlayerData.accounts[i].money+0.5) ..'$</span>'),
			...
			for i=1, #PlayerData.inventory, 1 do
				if PlayerData.inventory[i].count > 0 then
				    table.insert(elem, {
				      label     = (PlayerData.inventory[i].label .. '<span>' .. math.floor(PlayerData.inventory[i].count)..'</span>'),
		...
		for i=1, #weaponList, 1 do
		  ...
		  if HasPedGotWeapon(playerPed,  weaponHash,  false) and weaponList[i].name ~= 'WEAPON_UNARMED' then
			local ammo = 1
			table.insert(elem, {label = (weaponList[i].label .. '<span>' .. math.floor(ammo) .. '</span>'),name = weaponList[i].label, type = 'item_weapon', value = weaponList[i].name, count = ammo})
		  end
